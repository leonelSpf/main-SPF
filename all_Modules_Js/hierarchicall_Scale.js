export default function jerarquia() {
  ...
  fetch(url)
    .then((res) => (res.ok ? res.json() : Promise.reject(res)))
    .then((data) => {
      const { basicScale, oficial, inspector, comisary } =
        data.hierarchicalScale.grade;

      $boxGrades.setAttribute("class", "grades");
      $scaleGrades.appendChild($boxGrades);

      const createFigure = (scale, scaleId) => {
        const $scaleElem = d.getElementById(scaleId);
        scale.forEach((grade) => {
          const $containeFigure = d.createElement("figure"),
            $containeFigcaption = d.createElement("figcaption"),
            $containeImg = d.createElement("img");
          ...
          $containeFigure.setAttribute("class", "containerGrades");
          $containeImg.setAttribute("src", grade.gradeImg);
          $containeFigcaption.innerHTML = grade.gradeScale;

          $containeFigure.appendChild($containeImg);
          $containeFigure.appendChild($containeFigcaption);
          $scaleElem.appendChild($containeFigure);
        });
      };

      createFigure(basicScale, "basicScale");
      createFigure(oficial, "oficialScale");
      createFigure(inspector, "inspectorScale");
      createFigure(comisary, "ComisarioScale");
    })
    .catch((error) => {
